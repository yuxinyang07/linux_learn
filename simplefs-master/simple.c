...
void simplefs_inode_del(struct super_block *vsb, struct simplefs_inode *inode)
{
	...
	printk("%s %d ~  inode->inode_no = %d~~~~~~~~~~~~~~~~~~~~\n",__func__,__LINE__, inode->inode_no);
	...
	memset(inode_iterator, 0, sizeof(struct simplefs_inode));
	sb->inodes_count--;
 ...
 * will still be marked as non-free. You need fsck to fix this.*/
 int simplefs_sb_remove_block_inode(struct super_block *vsb, uint64_t  block_data_num, uint64_t  inode_num)
{
	int ret= 0;
	struct simplefs_super_block *sb = SIMPLEFS_SB(vsb);	
	if (mutex_lock_interruptible(&simplefs_sb_lock)) {
	...
	if(sb->free_blocks & ( 1 << block_data_num )){
		sfs_trace("block_data_num failed %d\n",block_data_num);
		ret = -EINTR;
		goto end;
	}
	sb->free_blocks  |= 1 << block_data_num ;
	if(sb->inodes_bitmap & ( 1 << (inode_num-1) )){
		sfs_trace("inode_num failed %d\n",inode_num);
		ret = -EINTR;
		goto end;
	}	
	sb->inodes_bitmap  |= 1 <<(inode_num -1) ;
	simplefs_sb_sync(vsb);
	end:
	mutex_unlock(&simplefs_sb_lock);
}
int simplefs_sb_get_a_freeblock_inode(struct super_block *vsb, uint64_t * out, uint64_t * num)
{
	struct simplefs_super_block *sb = SIMPLEFS_SB(vsb);
	int i;
	int ret = 0;
...
static int simplefs_delete_fs_object(struct inode *dir, struct dentry *dentry)
{
	...
	struct  inode *inode; 
	struct simplefs_inode *sfs_inode;
	struct simplefs_inode *parent_dri_inode;
	struct buffer_head *bh;
	...
	printk("simplefs_unlink%d %d   block num%d  len =%d %s  \n ",dir->i_ino,dentry->d_inode->i_ino, sfs_inode->data_block_number,sfs_inode->file_size,dentry->d_iname);
	/*clean inode datablock*/
	bh = sb_bread(dir->i_sb, sfs_inode->data_block_number);
	buff = (char *)bh->b_data;
	memset(buff,0,sfs_inode->file_size);
	mark_buffer_dirty(bh);
	sync_dirty_buffer(bh);
	brelse(bh);

	/*clean surperblock datablock*/
	simplefs_sb_remove_block_inode(dir->i_sb, sfs_inode->data_block_number, sfs_inode->inode_no);
	/*clean parant datablock name of inode*/
	parent_dri_inode = SIMPLEFS_INODE(dir);
	bh = sb_bread(dir->i_sb, parent_dri_inode->data_block_number);
	dir_contents_datablock = (struct simplefs_dir_record *)bh->b_data;
	for(i = 0;i < parent_dri_inode->dir_children_count; i++){
		if(dir_contents_datablock->inode_no == sfs_inode->inode_no){
			memset(dir_contents_datablock,0,(sizeof(struct simplefs_dir_record)));
			break;
		}
	...
	simplefs_inode_del(dir->i_sb, sfs_inode);
#if 1
	if (mutex_lock_interruptible(&simplefs_inodes_mgmt_lock)) {
	...
	parent_dri_inode->dir_children_count--;
	ret = simplefs_inode_save(dir->i_sb, parent_dri_inode);
	if (ret) {
		mutex_unlock(&simplefs_inodes_mgmt_lock);
		mutex_unlock(&simplefs_directory_children_update_lock);
...
//	sfs_inode->inode_no = inode->i_ino;
	 ...
	 */
	ret = simplefs_sb_get_a_freeblock_inode(sb, &sfs_inode->data_block_number,&inode->i_ino);
	if (ret < 0) {
	...
	for(i = 0;i< parent_dir_inode->dir_children_count + 1;i++){		
...
static int simplefs_rmdir(struct inode *dir, struct dentry *dentry)
{
	printk("simplefs_rmdir  %d \n ",dir->i_ino);
	return 0;
}
static int simplefs_unlink(struct inode *dir, struct dentry *dentry)
{
	simplefs_delete_fs_object(dir,dentry);
	return 0;
}
static int simplefs_mkdir(struct inode *dir, struct dentry *dentry,
			  umode_t mode)
...
{
	struct dentry *ret;
	struct simplefs_inode test;
	printk("%s %d \n~~~~~~~~~~~~~~~~~~~~~",__func__,__LINE__);
	printk("%s%d  size = %d\n",__func__,__LINE__,sizeof(test.mode));
