...
//    printk( "hym8563 rtc _irq:%d\n",hym8563->client->irq);
     gpio_int = of_get_named_gpio(client->dev.of_node,"rtc_int",0);
    printk( "hym8563 rtc _irq:%d\n",gpio_int);
     if(gpio_int< 0){
         printk("gpio_int get failed \n");

         return -ENOMEM;
     }
     irq_gpio = gpio_to_irq(gpio_int);
	 //client->irq=gpio_to_irq(gpio_int);
    printk( "hym8563 rtc _irq:%d\n",hym8563->client->irq);
	 printk( "hym8563 fdfd  rtc _irq:%d\n",irq_gpio);
        printk( "%d %s~~~~~~~~~~~~~~~~~~~:\n",__LINE__,__func__);
	if (client->irq > 0) {
		printk( "%d %s~~~~~~~~~~~~~~~~~~~:\n",__LINE__,__func__);
